...
class ImageClassifier(nn.Module):
    def __init__(self, pretask_model, feature_dim=10):
        ...
        #TODO: choose between these three
        self.predictor = nn.Sequential(nn.Linear(2048, 10, bias=True))
        #self.predictor.load_state_dict(torch.load('./classifier.pt', map_location=device)['model_state_dict'])
        #self.predictor = nn.Sequential(nn.Linear(2048,512, bias=True),  nn.BatchNorm1d(512),
        #                             nn.ReLU(inplace=True),nn.Linear(512,feature_dim, bias=True))
        #self.predictor = nn.Sequential(nn.Linear(2048,512, bias=True),  nn.BatchNorm1d(512),
        #                              nn.ReLU(),nn.Linear(512,128, bias=True),  nn.BatchNorm1d(128),   #Moc$
        #                              nn.ReLU(), nn.Linear(128, feature_dim, bias=True))
        #self.predictor.load_state_dict(torch.load('./clf_checkpoint.pt', map_location=device)['model_state_dict']) #remove this line
